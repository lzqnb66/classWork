...
const resultTimeRange = computed(() => {
  ...
  return `${minDate || '无数据'} 至 ${maxDate || '无数据'}`;
});
...
const executeQuery = async () => {
  try {
    ...
    // 执行查询（注意：advancedQuery 直接返回数据本体，而不是 { data } 包装）
    const data = await queryApi.advancedQuery(queryParams);
    queryResult.value = Array.isArray(data) ? data : (data?.data ?? []);
